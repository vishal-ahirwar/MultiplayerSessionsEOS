...
void UMenu::MenuSetup(int32 NumberOfPublicConnections, FString TypeOfMatch, FString LobbyPath)
{
	...
	if (MultiplayerSessionsSubsystem)
	{
		...
		MultiplayerSessionsSubsystem->multiplayerOnLoginCompleteDelegate.AddDynamic(this, &UMenu::onLoginComplete);
	}
	...
	if (subSystem)
	{
		...
		status->SetText(FText::FromString(FString::Printf(TEXT("Sub System found : %s"), *subSystem->Get()->GetSubsystemName().ToString())));
	}
}
...
void UMenu::OnCreateSession(bool bWasSuccessful)
{
	if (bWasSuccessful)
	{
		status->SetText(FText::FromString("Session has been created!"));
		UWorld* World = GetWorld();
		if (World)
		{
	...
	else
	{
		...
		status->SetText(FText::FromString("Failed to Create Session"));
...
void UMenu::OnFindSessions(const TArray<FOnlineSessionSearchResult>& SessionResults, bool bWasSuccessful)
{
	...
	for (auto Result : SessionResults)
	{
		...
		Result.Session.SessionSettings.Get(SEARCH_KEYWORDS, SettingsValue);
	...
	{
		status->SetText(FText::FromString("Failed to find Session"));
...
void UMenu::HostButtonClicked()
{
	...
	if (MultiplayerSessionsSubsystem)
	{
		status->SetText(FText::FromString("Host Button Pressed!"));
...
void UMenu::JoinButtonClicked()
{
	...
	if (MultiplayerSessionsSubsystem)
	{
		status->SetText(FText::FromString("Join Button Pressed!"));
